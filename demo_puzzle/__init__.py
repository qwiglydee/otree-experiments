...
def encode_trial(trial):
    # encoding initial board state
    return dict(difficulty=trial.difficulty, board=json.loads(trial.puzzle))


def validate_trial(trial, moves: list):
    ...
    board = json.loads(trial.puzzle)
    puzzle_utils.applyMoves(board, C.BOARD_SIZE, moves)
    correct = puzzle_utils.validateBoard(board)

    trial.actions = json.dumps(moves)
...
def on_solution(player, trial, solution, response_time):
    print("solution:", solution, "time:", response_time)

    trial.response_time = response_time

    if len(solution) == 0:
        trial.solution = None
        trial.is_timeouted = False
        trial.is_correct = None
