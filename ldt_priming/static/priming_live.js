...
class Model {
    constructor() {
        this.stimuli_slots = ['target', 'prime'];
        this.reset();
        this.progress = null;
    }
    ...
    setTrial(data) {
        this.stimuli.target = data.target;
        this.stimuli.prime = data.prime;
    }
...
class View {
    ...
    reset() {
        ...
        this.model.stimuli_slots.forEach(s => this.$stimuli[s].replaceChildren());
    ...
    _createStimulusElem(parent, tagname) {
    ...
    async _loadStimulus(slot, stimulus) {
        /** insert stimulus value in an appropriate place
        Returns: a promise resolving when content is loaded and ready to show
        */
        let parent=this.$stimuli[slot], elem = null;
            if (stimulus === null) {
                return Promise.resolve();
            }
            switch(stimulus.type) {
                case 'text':
                    elem = this._createStimulusElem(parent, "span");
                    elem.textContent = stimulus.text;
                    return Promise.resolve();
                case 'image-url':
                    elem = this._createStimulusElem(parent, "img");
    ...
    async loadStimuli() {
        /** load (all) stimuli data from model to view
        Returns: promise resolving when everything is loaded and ready to show
        */
        return Promise.all(
            this.model.stimuli_slots.map(s => this._loadStimulus(s, this.model.stimuli[s]))
        )
    }

    showStimulus(slot) {
        this.show(this.$stimuli[slot]);
    }

    hideStimulus(slot) {
        this.hide(this.$stimuli[slot]);
    }

    hideStimuli() {
        this.model.stimuli_slots.forEach(s => this.hideStimulus(s));
    }
...
class Controller {
    ...
    async displayTrial() {
        ...
        this.view.showStimulus('prime');
        await timers.sleep(PARAMS.cue_display_time);
        this.view.hideStimulus('prime');

        await timers.sleep(PARAMS.soa_time - PARAMS.prime_display_time);

        this.view.showStimulus('target');
