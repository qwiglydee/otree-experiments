...
TEXT_FONT = Path(__file__).parent / "assets" / "FreeSans.otf"
TEXT_SIZE = 32
...
def distort_image(img):
    ...
    for op in DISTORTIONS:
        if isinstance(op, ImageFilter.Filter):
            img = img.filter(op)
        if isinstance(op, ImageMorph.MorphOp):
            _, img = op.apply(img)

    # the distorsion may leave black border
